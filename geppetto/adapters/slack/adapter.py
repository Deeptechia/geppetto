...
class SlackAdapter(PlatformAdapter):
    ...
    def initialize(self) -> None:
        ...
        # This todo is the equivalent of the get_provider_info method in the LLMMultiplexer class
    ...
    async def get_or_create_conversation(
        ...
        conversation = Conversation.create(
            ...
            system_message=Message.system(self.personality),
        )
    ...
    async def _handle_event(self, body: Dict[str, Any]) -> None:
        ...
        slack_message = SlackMessage.from_event(event)

        # Check if user is allowed
        if (
            slack_message.user_id not in self.allowed_users
            and "*" not in self.allowed_users.values()
        ):
        ...
        if "llm_type" in conversation.metadata:
            ...
            # TODO: should we update the conversation metadata to not include the llm_type? so next time we don't have to check for it?

        # Process the message
        user_id = slack_message.user_id
        user_info = await self.get_user_info(user_id)

        message = SlackMessageMapper.slack_to_core_message(
            slack_message, metadata={"user_info": user_info}
        )
        ...
        if not user_name:
            try:
                ...
                if user_info_response["ok"]:
                    return SlackMessageMapper.extract_user_info(user_info_response)
            except SlackApiError as e:
                logging.error(f"Error fetching user info: {e}")

        # Fallback for allowed users or errors
        return UserInfo(
            id=platform_user_id,
    ...
    async def _generate_and_send_response(
        ...
        try:
            ...
            if isinstance(llm_response.content, ImageContent):
                ...
                slack_message = SlackMessageMapper.core_to_slack_message(
                    llm_response, conversation.metadata
                )
                slack_message.ts = processing_ts
                slack_message.text = append_version_to_message(
                    slack_message.text, self.llm_provider
                )
                slack_payload = slack_message.to_slack_payload()
                await self.app.client.chat_update(**slack_payload)
